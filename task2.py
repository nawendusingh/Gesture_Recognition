...
def ppr_based_classifer(query_gestures, labels_dict):
	gestures_list = pd.read_csv("gestures_list.csv", sep="/n")
	...
	ppr_values_new = {}
	"""
	for i in ppr_values.keys():
		if i in gestures_list:
			ppr_values_new[i] = ppr_values
			"""
...
def main():
	# k = int(input("Type 0 to use test set, type 1 to use your input: "))
	k=1
	labelDataPath = input("Enter the path of the labels: ")
	labelled_data = get_labelled_data("Outputs/pca_transformed.json", labelDataPath)
	# print(labelled_data)
	# print(labelled_data.loc['1'].tolist()[:-1])
	train_set, test_set = get_train_test_data(labelled_data)
	train_set_np = train_set.to_numpy()
	test_set_np = test_set.to_numpy()
	labels_dict = get_labels_dict(labelDataPath)
	# print(labels_dict)
	train_X, train_y, test_X, test_y = get_features_labels_seperately(train_set, test_set)
	# print(type(train_X))
...
def task2_main(k=1, query_list=["1"]):
	...
	train_set_np = train_set.to_numpy()
	test_set_np = test_set.to_numpy()
	path_name = input("Enter the path_name of labels file: ")
	labels_dict = get_labels_dict(path_name)
	train_X, train_y, test_X, test_y = get_features_labels_seperately(train_set, test_set)
